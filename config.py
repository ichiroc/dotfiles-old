...
##
## Windows の操作を emacs のキーバインドで行うための設定（Keyhac版）ver.20160923_01
##
...
def configure(keymap):
    ...
    window_movement_key = [] # Multi-display
    ...
    def toggle_input_method():
        ...
        if use_emacs_ime_mode:
            keymap_emacs_ime.ime_status = ime_status
    ...
    if use_emacs_ime_mode:
        def self_insert_command2(*keys):
            ...
            def _func():
                func()
                if keymap_emacs_ime.ime_status:
...
#    define_key(keymap_emacs, "M-d",      reset_search(reset_undo(reset_counter(reset_mark(repeat3(kill_word))))))
    ...
    if use_emacs_ime_mode:
        ...
        def is_emacs_ime_mode(window):
            keymap_emacs_ime.ime_status = window.getImeStatus()
        ...
        # IME の状態を格納する
        keymap_emacs_ime.ime_status = False
